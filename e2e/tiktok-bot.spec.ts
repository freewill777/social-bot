...
test("Tiktok bot", async ({ page }) => {
  await setUp(page);

  while (true) {
    console.log("Entering main bot loop");
...
async function setUp(page) {
  ...
  var username = config.tiktok.username;
  var password = config.tiktok.password;
  await page.goto(url);

  await clickIfPresent(page, 'button:has-text("Log in")');
  await clickIfPresent(page, 'p:has-text("Use phone / email / username")');
  await clickIfPresent(page, 'a:has-text("Log in with email or username")');
